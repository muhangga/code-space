   ...
   navigator.geolocation.getCurrentPosition(position => {
      ...
      let map = new H.Map(
         ...
         defaultLayers.vector.normal.map,
         {
            ...
            pixalRatio: window.devicePixelRatio || 1
         });
         ...
         if (window.action == "browse") {
          ...
          function onError(error) {
              alert("Tidak bisa di akses!" + error);
          }
          ...
          function addRouteShapeToMap(route) {
              ...
              //   Menambahkan objek polyline ke dalam map
              map.addObjects([polyline, routeline, startMarker, endMarker]);

              // ngezoom ke objek kotaknya
          ...
          function addSummaryToPanel(summary) {
              ...
              const markup = `
                <ul>
                    ...
                    <li>Waktu Tempuh: ${summary.travelTime.toMMSS()} (sesuai kondisi lalu lintas saat ini)</li>
                </ul>
            `;
...
// Buka url lokasi
function openlokasi(lat, lng, id) {
   window.open(
